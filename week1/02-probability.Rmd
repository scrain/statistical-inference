...
* The probability of at least one of multiple things that are mutually exclusive occur is the sum of their probabilities
...
Area must be one for a PDF and the area of a triagle is $(h \times w)/2$.
...
Somewhat important quote from swirl
...
Surivor function is the compliment of the CDF
...
2. I define a PMF, pp so that for $x=0$ and $x=1$ we have $p(0)=−0.1$ and $p(1)=1.1$. Is this a valid PMF?

No, it is invalid because the probabilies cannot be < 0
...
6. A random variable, $X$, is uniform, a box from 0 to 1 of height 1. (So that it’s density is $f(x)=1$ for $0≤x≤1$.) 
...
$F(x) = P(X≤x) = Base \times Height = x$

Median (0.5) would be for $x = 0.50$
